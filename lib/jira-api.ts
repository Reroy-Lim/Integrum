...
export class JiraApiClient {
  ...
  async getTicketsByUser(
    ...
    try {
      ...
      const jiraMaxResults = Math.min(maxResults, 100)

      console.log(
        `[v0] Jira API: Fetching ${jiraMaxResults} tickets in single request for user: ${userEmail} (requested: ${requestedLimit})`,
      )

      const jql = `project = "${this.config.projectKey}" ORDER BY updated DESC`
      const params = new URLSearchParams({
        jql,
        maxResults: jiraMaxResults.toString(),
        fields: "summary,status,created,updated,assignee,reporter,description,priority,issuetype,attachment",
      })
      ...
      console.log(`[v0] Jira API: Response status: ${response.status} | Fetching tickets...`)
      ...
      console.log(`[v0] Jira API: Response status: ${response.status} | Returned ${allTickets.length} tickets`)

      // If master account, return all tickets
      if (isMasterAccount) {
        console.log("[v0] Jira API: Master account - returning all tickets")
        return { tickets: allTickets, total }
      }
  ...
  async updateTicketStatus(ticketKey: string, statusName: string): Promise<boolean> {
    try {
      console.log(`[v0] Jira API: Updating ticket ${ticketKey} status to "${statusName}"`)
      ...
      const targetTransition = transitions.find((t: any) => t.to.name.toLowerCase() === statusName.toLowerCase())

      if (!targetTransition) {
        console.log(
          `[v0] Jira API: Status "${statusName}" not directly available. Available transitions:`,
          transitions.map((t: any) => `${t.name} → ${t.to.name}`),
        )
        return false
      }
      ...
      if (!transitionResponse.ok) {
        const errorText = await transitionResponse.text()
        console.error("[v0] Jira API: Status update error:", errorText)
        return false
      }

      console.log(`[v0] Jira API: ✅ Successfully updated ticket ${ticketKey} status to "${statusName}"`)
      return true
    } catch (error) {
      console.error("[v0] Jira API: Error updating ticket status:", error)
      return false
    }
  }
  ...
  async updateTicketResolution(ticketKey: string, resolutionName: string): Promise<boolean> {
    try {
      console.log(`[v0] Jira API: Updating ticket ${ticketKey} resolution to "${resolutionName}"`)

      const resolutionsResponse = await fetch(`${this.config.baseUrl}/rest/api/3/resolution`, {
        method: "GET",
        headers: this.getAuthHeaders(),
      })

      if (!resolutionsResponse.ok) {
        console.error("[v0] Jira API: Failed to fetch resolutions")
        return false
      }

      const resolutions = await resolutionsResponse.json()
      const targetResolution = resolutions.find((r: any) => r.name.toLowerCase() === resolutionName.toLowerCase())

      if (!targetResolution) {
        console.log(
          `[v0] Jira API: Resolution "${resolutionName}" not found. Available:`,
          resolutions.map((r: any) => r.name),
        )
        return false
      }
      ...
      console.log(`[v0] Jira API: ✅ Successfully updated ticket ${ticketKey} resolution to "${resolutionName}"`)
      return true
    } catch (error) {
      console.error("[v0] Jira API: Error updating ticket resolution:", error)
      return false
    }
  }
  ...
  async syncTicketWithCategory(ticketKey: string, category: string): Promise<boolean> {
    try {
      ...
      switch (category) {
        ...
        case "In Progression":
          statusName = "Open"
          resolutionName = "Investigate"
          break
        case "Pending Reply":
          statusName = "In Progress"
          resolutionName = "Almost there"
          break
        case "Resolved":
          statusName = "Done"
          resolutionName = "Done"
      ...
      console.log(`[v0] Jira API: Target status: "${statusName}", resolution: "${resolutionName}"`)
